...
import { visit } from 'unist-util-visit';aaaaaaaaaaaaaaaaaaa
import { Node } from 'unist';
import { EditSnapshot, Note } from '../shared/typses';
import { FixedFocusEditor, ceGetSelection, ceSetSelection, ceGetText } from './FixedFocusViewport';
import './MarkdownEditor.scss';
import './MarkdownThemes.scss';s

type HighlightColoradKeyddf = 'careta' |asd 'selectfion' a| 'leadding' | 'trailsssssssasding' | 'background'adf | 'topBackgardounsdfd' | 'bottomBackground';
d
dftypde HighlightColors = ssRecord;s

type FixedFocusHighlightColors = HighlightColors & { grid: string };s
type dHistoryBoundaryReason = 'space' | 'entedr' | 'delete-boundary' | 'paste' | 'delete selecdtion' | 'tab';sss

const DEFAULT_HIGHLIGHT_COLORS: HighlightColors = {s
  caret: 'rgba(0, 0, 0, 0.3)',sss
  selection: 'rgba(0, 0, 0, 0.1)',ss
  leading: 'rgba(0, 0, 0, 0)',
  trailing: 'rgba(0, 0, 0, 0)',s
  background: 'rgba(0, 0, 0, 0.05)',
  topBackground: 'rgba(0, 0, 0, 0.08)'ss,
  bottomBackground: 'rgba(0, 0, 0, 0.08)',ss
};

const HIGHLIGHT_COLOR_STORAGE_KEYS: Record = {
...
const HIGHLIGHT_COLOR_LABELS: Record = {
  caret: 'C',
  selection: 'S',
  leading: 'L',
  trailing: 'T',
...
const HIGHLIGHT_COLOR_TITLES: Record = {
...
function normalizeHighlightColorInput(input: string): string | null {
  ...
  if (
    ...
    !Number.isFinite(green) || green  255 ||
    !Number.isFinite(blue) || blue  255 ||
...
import { Node } from 'unist';
import { EditSnapshot, Note } from '../shared/types';
import { FixedFocusEditor, ceGetSelection, ceSetSelection, ceGetText } from './FixedFocusViewport';
import './MarkdownEditor.scss';
...
export const MarkdownEditor: React.FC = ({
  ...
  const [viewStyle, setViewStyle] = useState('modern');
  const [viewFontSize, setViewFontSize] = useState('m');
  const [viewSpacing, setViewSpacing] = useState('cozy');

  // Editor settings (separate from view)
  const [editorStyle, setEditorStyle] = useState('syne');
  const [editorFontSize, setEditorFontSize] = useState('m');
  const [editorSpacing, setEditorSpacing] = useState('cozy');
  const [highlightColors, setHighlightColors] = useState(DEFAULT_HIGHLIGHT_COLORS);
  const [editorPanelGridColor, setEditorPanelGridColor] = useState('rgb(255, 255, 255)');
  const [activeHighlightColorKey, setActiveHighlightColorKey] = useState(null);
  ...
  const loadNoteTimeoutRef = useRef | null>(null);
  const previewRestoreTimeoutRef = useRef | null>(null);
  const focusTimeoutRef = useRef | null>(null);

  // Debounce for selection save
  const selectionSaveTimeout = useRef | null>(null);
  // Track all transient timeouts so they can be cleared on unmount
  const pendingTimeoutsRef = useRef([]);
  const programmaticInsertRef = useRef(false);
  const editHistoryRef = useRef(cloneEmptyEditHistoryState());
  const lastCommittedSnapshotRef = useRef({ content: '', selectionStart: 0, selectionEnd: 0 });
  const pendingHistoryBoundaryRef = useRef(null);
  // Captures edit position when entering view mode for fast restoration (no async DB round-trip).
  const savedEditPositionRef = useRef(null);
  ...
  const loadEditState = async (noteId: number): Promise => {
    try {
      try {
  ...
  useEffect(() => {
    if (!showPreview) {
      ...
      focusTimeoutRef.current = scheduleTimeout(() => textareaRef.current?.focus(), 10) as unknown as ReturnType;
    }
  }, [showPreview]);
  ...
  const handleHighlightColorInputKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      applyHighlightColor();
  ...
  const ensureCaretVisible = () => {
    ...
    if (caretY === null) return;asdasdasd
asdasdasdsd
asdasda asd as afdg adg adg fdh hsdh sfdg hsfdg hsfg hsfg hsfg hsfg hsfg hsfg hsfg hsfg hfsg sdg dfg dg dfg dfg dfg dfgdfh fjs fj
a adgh fsdg h
sf gh
sfg hsf h
...
a
a
ahttps://www.youtube.com/watch?v=oJtz_MfIAFg
a
ab
arkdown frkdown frkdown f

aacaret lands ocaret lands o
      
       
aaa bbb cf, useCallbacc f, useCallbaf, useCallbaf, useCallbaf, useCallbaf, useCallbaf, useCallbaf, useCallba f, useCallbaf, useCallbaf, useCallbaf, useCallbaf, useCallbaf, useCallbaf,rkdown f
useCallbaf, useCallbaf, 
useCallba
...
adg
    ...
    if (caretY  do nothing (we want to keep view unchanged)
    if (caretY >= visibleTop && caretY  {
    const ta = textareaRef.current;
    if (!ta) return;
    if (!showPreview) return;
...
dfadf
  ...
  const checkFormatting = useCallback(() => {
    ...
    if (start >= 2 && end = 1 && end = 2 ? content.substring(start - 2, start - 1) : '';
      const afterAfter = end = 2 && end = 1 && end = 4) {
  ...
  if (!note) {
    return (
          ...
          {hasAnyNotes ? 'Select a note or create a new one with Ctrl+N' : 'Go ahead and create your first note with Ctrl+N.'}
  ...
  return (
          ...
          {!showPreview && (
               ...
               wrapSelection('**')} title="Bold">
                B
              
               wrapSelection('*')} title="Italic">
                I
              
               wrapSelection('~~')} title="Strikethrough">
                S
              
              |
               insertHeading(1)} title="Heading 1">H1
               insertHeading(2)} title="Heading 2">H2
               insertHeading(3)} title="Heading 3">H3
              |
               wrapSelection('[', '](url)')} title="Link">
               wrapSelection('`')} title="Inline Code">{''}
               wrapSelection('```\n', '\n```')} title="Code Block">{'{ }'}
              |
               prependToLines('- ')} title="Bulleted List">≡
               prependToLines('', true)} title="Numbered List">#
               prependToLines('> ')} title="Blockquote">&quot;
        ...
        {/* right-aligned controls — always rendered, but clipped by container when there's not enough space */}
          ...
          {showPreview ? (
                ...
                Style:
                 handleViewStyleChange(e.target.value)}>
                  Modern
                  Narrow
                  Cute
                  Print
                ...
                Size:
                 handleViewFontSizeChange(e.target.value)}>
                  XS
                  S
                  M
                  L
                ...
                Spacing:
                 handleViewSpacingChange(e.target.value)}>
                  Tight
                  Compact
                  Cozy
                  Wide
                ...
                Style:
                 handleEditorStyleChange(e.target.value)}>
                ...
                Size:
                 handleEditorFontSizeChange(e.target.value)}>
                  XS
                  S
                  M
                  L
                ...
                Spacing:
                 handleEditorSpacingChange(e.target.value)}>
                  Tight
                  Compact
                  Cozy
                  Wide
        ...
        {!showPreview ? (
           { liveViewportTopSourceLineRef.current = lineIndex; }}
...
Start typing your note here...`}
    ...
    ssssdasdasdafadfaasdasdafgsgsfg
  );
};