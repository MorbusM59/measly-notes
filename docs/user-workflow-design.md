...
**Naming rule.** Anything meaning the CONTAINER says `slot` — identifiers, comments, tooltips, docs. That pass is done: `isLeftmostSlot`, `canCreateSlot`, `onCreateSlot`, `onCloseSlot`, `handleCreateSlot`, `handleCloseSlot`, `SLOT_MIN_WIDTH_PX`, and all of `shared/slotWidths.ts` — formerly `sectionWidths.ts`, which was pure slot geometry living under a collection's name.
...
**Two kinds of new note, and the second one is the point.** Creating a note from a section's own create pill means *I know where this goes* — it is born in that section, pinned, done. Creating one from the shortcut or the quick-actions menu means *someone is on the phone* — the user has no attention to spare for where it belongs, and the app must not ask. A note born that way is **undocked**: it belongs to no section, shows a blank section identity tab and an empty tab strip, and prompts for nothing at all. Left-clicking the blank identity tab populates the bar with the ordinary section picker; picking a section there — or the picker's own create button — pins the note into it. Never committing is a first-class outcome: the note lives in the sidebar, in no workspace, which is exactly what §1.2 says sections are for. Clicking any other note leaves the undocked one behind uncommitted and returns the slot to its section, so deferring costs nothing and is reversible.

Left-click files it; **right-click sets it aside** — dismissing the undocked note and bringing the slot's own section straight back. The pair reads as one decision with two answers, on the same blank tab. It is cheap precisely because undocked is presentation only: the section never left the slot, so restoring it means clearing a marker, not reloading anything. **Setting aside deletes the note only when it is still empty** — nothing but the `'# '` template the shortcut created. That is the interrupted-before-typing case, where keeping it would leave a blank note in the sidebar forever and nothing is lost by removing it. For a note with any content at all, setting aside is purely "stop showing it here": it stays saved, keeps its id, and is reachable from the sidebar like any other note. Dismissal is never a way to discard writing, and the tooltip must say "set aside", never "close" or "discard". The gesture is an accelerator, not a rescue: clicking any note in the sidebar already restores the section, and *that* is what actually keeps the user from being stuck.

The internal/external split is the delicate part, and it is already clean: internally every note is displayed *in* a slot, which holds a section, but the global create path simply never pins — so an undocked note is genuinely absent from every section's tab list rather than being hidden from one. Undocked is a presentation state plus commit gestures; it changes nothing in the data model.
...
**A bar can have a resting layer, and the resting layer is where the nudge lives.** The chapter bar holds a note's two layers -- metadata (tags, note id) and content structure (chapters) -- and returns to one of them whenever the user clicks back into the text. Which one is the entire enforcement of "commit to an id before the note grows structure": while a note's id is still provisional (`NOTE-#n`, see `shared/assignedIds.ts`) the bar rests on **metadata**; once the note has an id of its own it rests on **chapters** and never prompts again. Nothing is ever blocked, the other layer is always one click away and stays put for as long as the user is actually there, and committing an id never flips the bar mid-edit -- the resting layer only changes where the bar comes back to. This is the shape to reach for when a concept needs to be taught rather than enforced: make the thing still asking for something the place the UI returns to, and let it stop asking the moment it is satisfied.
